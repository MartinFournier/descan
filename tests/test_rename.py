...
def test_clean_original_strip_tokens():
    assert R.clean_original("nickname_3x_beach_p02", ("nickname",)) == "beach"
    assert R.clean_original("a_nickname_b", ("nickname",)) == "a_b"
...
def test_split_applied_prefix_none_when_not_applied():
    date, base = R.split_applied_prefix("nickname_beach_p01", "Person")
    assert date is None
    assert base == "nickname_beach_p01"
